...
class ProtoNet_Attention(nn.Module):
    ...
    def __init__(self, x_dim=1, hid_dim=512, z_dim=64):
        ...
        self.linear1 =  nn.Linear(7*7*256,256)
        self.LSTM = nn.Sequential(
            nn.LSTM(input_size,hidden_size,num_layer,batch_first=True,bidirectional=False) #,bidirectional=True
        )
        self.GRU = nn.Sequential(
            nn.GRU(input_size,hidden_size,num_layer,batch_first=True,bidirectional=True) #,bidirectional=True,LSTM
        )
...
#         return context, alpha_n
    ...
    def attention(self, lstm_output, final_state):
        ...
        return torch.bmm(torch.transpose(lstm_output, 1, 2), weights).squeeze(2), weights.squeeze(2)
...
#         x = self.linear1(x.view(x.size(0), -1))
        ...
        #注意力
        x, hidden = self.GRU(x)
        attn_output, weights = self.attention(x, hidden)
...
def test_LSTM_prelabel(test_bytecodes, vul_use_names, model_save_dir, kind_size_set, max_len_set, batchsize_set):
    ...
    for vul_use_name in vul_use_names:
        ...
        def test(dataloader):
            ...
            with torch.no_grad():   #使得以下代码执行过程中不用求梯度
                for batch_idx,data in enumerate(dataloader):
                    ...
                    if torch.cuda.is_available():
                        ...
                        # correct+=(predicted==labels).sum().item()
                        weights_all.extend(weights.cpu().numpy())
                    else:
                        predict_labels.extend(predicted.numpy())
                        # true_labels.extend(labels.cpu().numpy())
                        class_bags.extend(bags.numpy())
                        # correct+=(predicted==labels).sum().item()
                        weights_all.extend(weights.numpy())
            ...
            for bag,x in group:
                ...
                if (x['prelabels']==1).any():
                    ...
                    predict_labels.append(0)
                instances_predict_weights.append(weights_all[x.index.tolist()].tolist())
        ...
        #     print('Accuracy on test set:%d %%' % (100*correct/total))
            ...
            # print(predict_labels)
            # return true_labels, predict_labels
            return predict_labels, instances_predict_labels, instances_predict_weights
        
        model.eval()
        # true_labels, predict_labels = test(DataLoader_test)
        predict_labels, instances_predict_labels, instances_predict_weights = test(DataLoader_test)

        predict_results[vul_use_name] = {'predict_labels': predict_labels, 'instances_predict_labels':instances_predict_labels, 'instances_predict_weights':instances_predict_weights}

        del predict_labels, model, instances_predict_labels, instances_predict_weights
        gc.collect()
