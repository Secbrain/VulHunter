...
class SolidityMetadata:
    ...
    def __build_source_map(self, bytecode, srcmap): #映射字节码pc和源码的内容
        # https://solidity.readthedocs.io/en/develop/miscellaneous.html#source-mappings
        new_srcmap = {}
        bytecode = self._without_metadata(bytecode) #运行runtime的bytecode
        if self.source_code and bytecode and srcmap:
            ...
            pos_to_offset = {}
            for i in EVMAsm.disassemble_all(bytecode): #可能会跳 间隔出现 看指令个数
    ...
    def get_source_for(self, asm_offset, runtime=True):
        ...
        If runtime is False, initialization bytecode source map is used ASM位置到源码连接
        """
