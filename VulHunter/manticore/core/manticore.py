...
class ManticoreBase(Eventful):
    ...
    def _fork(
        ...
        solutions = state.concretize(expression, policy, explicit_values=values) #这块有个策略
        ...
        if len(solutions) == 1:
            ...
            for new_value in solutions: #new_value
                with state as new_state:
                    new_state.constrain(expression == new_value) #添加约束

                    # and set the PC of the new state to the concrete pc-dest
                    # (or other register or memory address to concrete)
                    setstate(new_state, new_value) #新状态的加入，所以看来manticore是一个状态state维护搜索执行
            ...
            logger.debug("Forking current state %r into states %r", state.id, children) #状态和子状态 得中间状态是不是子状态才有探索
    ...
    def finalize(self):
        ...
        """
        self.kill()
        for state in self.all_states: #先把这个注释 看生成文件 还是能
