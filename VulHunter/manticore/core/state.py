...
class Concretize(StateException): #通过先前舍弟年的策略，然后选择的State数值
...
class StateBase(Eventful):
    ...
    def concretize(self, symbolic, policy, maxcount=7, explicit_values: Optional[List[Any]] = None): #求解满足约束状态的值 根据预先设置的策略
    ...
    def is_feasible(self): #总得输入一个东西 那就True不影响其他元素
        return self.can_be_true(True)

    def can_be_true(self, expr):
        expr = self.migrate_expression(expr) #加入expression然后判断求解
    ...
    def solve_one_n(self, *exprs: Expression, constrain: bool = False) -> List[int]: #运行好几个输入内容求解
        """
