...
class RecordSymbolicBranches(Plugin): #状态的最后一个pc和目标跳跃pc 一个状态就是一个block 记录分支pc状态
    def did_execute_instruction_callback(self, state, last_pc, target_pc, instruction):
        if state.context.get("forking_pc", False): #不是分支节点
            branches = state.context.setdefault("branches", {})
            branch = (last_pc, target_pc)
            if branch in branches: #加入路径开始和末尾
                branches[branch] += 1
            else:
                branches[branch] = 1
