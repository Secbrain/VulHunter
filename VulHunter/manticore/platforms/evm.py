...
class EVM(Eventful):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if bytecode is not None and not issymbolic(bytecode):
            ...
            # print("bytecode_lenth", bytecode_size)
            # print("bytecode", bytecode_symbolic._concrete_cache)

        # TODO: Handle the case in which bytecode is symbolic (This happens at
        # CREATE instructions that has the arguments appended to the bytecode) CREATE指令会将参数添加到字节码中
        ...
        for i in EVMAsm.disassemble_all(extend_with_zeroes(bytecode)):
            if i.mnemonic == "JUMPDEST": #断开块，这个是跳转块的第一个地址，其他跳转jump或者jump除了顺序然后连接它
    ...
    def _checkpoint(self): #返回当前执行的命令
        ...
        if self._checkpoint_data is None: #新的指令 旧的执行完赋为False
    ...
    def _check_jmpdest(self):
        ...
        """
        # If pc is already pointing to a JUMPDEST thre is no need to check. 判断条件为True的 下一个是否是JUMPDEST
        pc = self.pc.value if isinstance(self.pc, Constant) else self.pc #常量直接取值
        if pc in self._valid_jumpdests: #跳转 则不检查是不是新块
            self._need_check_jumpdest = False
            return
        ...
        if isinstance(should_check_jumpdest, Constant):
            ...
            ) #求解出来的跳转地址？ 得到跳转得到的输入值 should_check_jumpdest True 或者 False
            self._publish(
                "did_solve",
                self.constraints,
            ...
            if len(should_check_jumpdest_solutions) != 1: #求解出来的跳转应该就有一个值
        ...
        # only to True, process it and also set it to False 只能True或者只能False都是False
        self._need_check_jumpdest = False

        if should_check_jumpdest: #True就退出
            if pc not in self._valid_jumpdests: #肯定不在
                self._throw()
    ...
    def _advance(self, result=None, exception=False):
        ...
        if not exception:
            ...
            #new add
            self.excute_instruction = last_instruction

            if not last_instruction.is_branch: #不是跳转的话 就是默认 加上最后一个指令大小
                # advance pc pointer
                self.pc += last_instruction.size
            self._push_results(last_instruction, result) #放回栈中
        self._publish("did_evm_execute_instruction", last_instruction, last_arguments, result) #执行的指令和参数
        self._checkpoint_data = None
        self._published_pre_instruction_events = False
    ...
    # Execute an instruction from current pc 如果不是常量 则是可能的两个值 都有可能跳转？
    def execute(self):
        pc = self.pc
        
        # print("bytecode", self.bytecode._concrete_cache)
        # print(self.evmfork)
        # print("data", self.data.array.value_bits) #self.disassemble()
        # print("bytecode", self.bytecode.array._operands[0].array) #self.disassemble()
        # return
        if issymbolic(pc) and not isinstance(pc, Constant): #对象是否是符号的（例如检查从内存读取的数据是否是符号的）不是常量 都是bool判断？ 所以是跳转的，需要判断跳转路径，可能能根据这个还原CFG
            expression = pc
            # print("execute symbolic")
            # print(expression.value) #pression.value 是静态constant才行 boolvalue不行
            # print([hex(xx.value) if isinstance(xx, Constant) else xx for xx in expression.condition.operands]) #BitVecITE 一个pc值 condition, true_value, false_value
            # print(expression.true_value.value) #true和false 后者继承
            # print(expression.false_value.value)
            ...
            raise Concretize("Symbolic PC", expression=expression, setstate=setstate, policy="ALL") #调用了这个 policy是ALL 遍历状态
        # else:
            # print("execute constant")
            # expression = pc
            # print(expression)

        # print("instruction--", self.instruction) #一样的 在self._checkpoint()也是这样取的

        #constant的执行 符号状态已退出
        try:
            self._check_jmpdest() #查看跳转
            # print(self.pc, "begin_stack", self.stack)
            last_pc, last_gas, instruction, arguments, fee, allocated = self._checkpoint() #下一个？这个看看 好像是求约束得来的
            result = self._handler(*arguments)
            # print(result)
            # print("last_pc", last_pc) #刚才执行完的pc
            
            # print("instruction", instruction)
            # print("arguments", arguments) #栈执行的参数
            
            # print("result", result) #得到的结果
            # print(self.pc, "after_stack", self.stack) #把栈拿出来，后面才放进去的
            # if issymbolic(self.pc): #self.pc是不变的
            #     print("pc_after", [self.pc.true_value.value, self.pc.false_value.value]) #self.pc 没有变化 是在哪里加下一个的，_advance()?
            # else:
            #     print("pc_after", self.pc)

            self._advance(result)
            # if issymbolic(self.pc): #self.pc也没变 在哪变得？
            #     print("pc_after_advance", [self.pc.true_value.value, self.pc.false_value.value]) #self.pc 没有变化 是在哪里加下一个的，_advance()?
            # else:
            #     print("pc_after_advance", self.pc)
        ...
        # if issymbolic(self.pc): #这会儿变了，EVM内容变了，所以它能重新这个函数
        #     print("pc_after_raise", [self.pc.true_value.value, self.pc.false_value.value]) #self.pc 没有变化 是在哪里加下一个的，_advance()?
        # else:
        #     print("pc_after_raise", self.pc)
        # print("---------")
    ...
    def ISZERO(self, a):
        """Simple not operator"""
        # print("_____")
        # print('ISZERO',self.excute_instruction.name != "ISZERO")
        if self.excute_instruction.name not in ['ISZERO', 'DUP1']:
            # print(Operators.ITEBV(256, a == 0, 1, 0) == 0)
            self.constraints.add(Operators.ITEBV(256, a == 0, 1, 0) == 0)
        # print(a)
        # print(self.excute_instruction)
        # print(self.instruction)
        # print("_____")
    ...
    def CALLDATALOAD(self, offset):
        ...
        calldata_overflow = None  # 32 地址溢出攻击
    ...
    def JUMP(self, dest):
        ...
        self._set_check_jmpdest() #默认跳转，下一个指令大小
    ...
    def JUMPI(self, dest, cond):
        ...
        self.pc = Operators.ITEBV(256, cond != 0, dest, self.pc + self.instruction.size) #True 得到下一个顺序pc
        # This set ups a check for JMPDEST in the next instruction if cond != 0
        self._set_check_jmpdest(cond != 0) #判断能不能跳转 True False可能计算
